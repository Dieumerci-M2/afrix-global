...
import { Code2, Megaphone, Palette, Settings, Building2, HeartHandshake, Landmark, Globe2 } from "lucide-react"

export const metadata: Metadata = {
  title: "Services — Afrix Global",
  description: "Digital execution powered by African talent: development, marketing, design, and custom solutions.",
}
...
const serviceGroups = [
  ...
  },
  {
    ...
    items: ["Internal Systems", "NGO Solutions", "Enterprise Platforms", "Digital Transformation"],
  },
]
...
export default function ServicesPage() {
  return (
    <>
      ...
      <Section>
        <MediaZone src="/images/services.png" alt="Afrix Global designers and developers at work" aspect="aspect-[16/9]" />
      </Section>

      <Section className="bg-card/40">
        <SectionHeading eyebrow="What we do" title="Full-service digital execution" />
        <div className="mt-10 grid gap-px overflow-hidden rounded-sm border border-border bg-border md:grid-cols-2">
          {serviceGroups.map((g) => (
            <div key={g.title} className="bg-background p-8">
              ...
              <h3 className="mt-6 text-lg font-semibold text-foreground">{g.title}</h3>
      ...
      <Section>
        <SectionHeading eyebrow="Who we serve" title="Trusted by organizations of every kind" />
        <div className="mt-10 grid gap-px overflow-hidden rounded-sm border border-border bg-border sm:grid-cols-2 lg:grid-cols-4">
          {clients.map((c) => (
            <div key={c.label} className="flex flex-col items-center gap-4 bg-background p-10 text-center">
